...
/*   Updated: 2023/02/09 18:44:14 by hnoguchi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_result_redirects(t_node *list, const char *message)
{
	...
	while (list != NULL)
	{
		...
		{
			printf(GREEN"NODE_REDIR_OUT"END);
			printf("\n");
			print_result_token(list->args, "[node->redirects->args]");
			printf("\n");
		}
		list = list->next;
	}
...
void	print_result_parse(t_node *node, char *message)
{
	...
	while (node != NULL)
	{
		...
		{
			printf(GREEN"NODE_SIMPLE_CMD"END);
			printf("\n");
			print_result_token(node->args, "[node->args]");
			printf("\n");
			print_result_redirects(node->redirects, "[node->redirects]");
			printf("\n");
		}
		node = node->next;
	}
...
int	main(void)
{
	...
	status = 0;
	token = tokenize("echo \"hello\" \"hello world\" > hello.txt >f2>\"f3\"");
