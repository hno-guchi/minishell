...
/*   Updated: 2023/02/26 16:49:32 by hnoguchi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

t_token	*new_token(t_token_kind kind, char *word, int last_status)
{
	t_token	*new_token;
	...
	new_token->word = word;
	new_token->kind = kind;
	new_token->last_status = last_status;
	new_token->next = NULL;
	return (new_token);
}

t_token	*new_token_type_operator(int last_status, char **rest, char *line)
{
		...
		if (is_operator(line, operators_list[i]))
		{
			...
			else
			{
				*rest = line + strlen(operator);
				return (new_token(TK_OPERATOR, operator, last_status));
			}
		}
		i += 1;
	}
...
t_token	*new_token_type_word(int last_status, char **rest, char *line)
{
	const char	*start_p;
	char		*word;
	...
	{
		*rest = line;
		return (new_token(TK_WORD, word, last_status));
	}
